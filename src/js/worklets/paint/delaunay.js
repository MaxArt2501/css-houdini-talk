...
registerPaint('delaunay', class {
  ...
  paint(ctx, geometry, props) {
    this.options = {
      ...
      lineWidth: 0,
      ...getOptionObject(props)
    };
    const points = [
      ...this.generatePoints(geometry),
      ...this.generateBorderPoints(geometry)
    ];
  ...
  generatePoints({ width, height }) {
    ...
    const length = Math.floor(area / this.options.pointArea);
  ...
  generateBorderPoints({ width, height }) {
    const perimeter = (width + height) * 2;
    const linearWidth = this.options.pointArea ** .5 / 2;
  ...
  drawTriangles(ctx, points, triangles) {
    ...
    for (let i = 0; i < triangles.length; i += 3) {
      ...
      const angle = this.getMedianAngle(a, b, c);
...
function getOptionObject(props) {
  ...
  props.forEach((value, key) => {
    ...
    if (stringValue) {
      map[propToOption(key)] = stringValue;
    }
  });
  return map;
}
...
function propToOption(name) {
  return name.replace(/^--delaunay-/, '').replace(/-[a-z]/g, m => m[1].toUpperCase());
}

// THE FOLLOWING IS NOT MY CODE
// From the awesome Delaunator v3.0.2
// https://github.com/mapbox/delaunator/blob/master/index.js
